...
- db/init 全 9 ファイル（09-updated-at-triggers.sql 追加後。**2026-07-27 現在は 10-attendance.sql を加えた全 10 ファイル**）を再初期化適用、
  ...
  `punch_records` の追記専用、勤怠 6 テーブルの `tenant_isolation` + FORCE 配線と
  **USING / WITH CHECK 式が `brands` と一致すること**、行のある勤怠テーブルの
  他テナント / GUC 未設定でのフェイルクローズ）。
  **勤怠 6 表は初期状態でほぼ空**（`leave_types` のシード 1 行のみ）で、行の無い表では
  「0 行」に検証としての意味が無い（分離を無効化しても 0 行）ため、**実データによる実証は
  行のある表に限り、空の表は `SKIP` として名前を挙げる**（分離の担保は式の一致側で行う）。
  出力が実際の検証範囲を上回って断定しないための措置（原則4）。
