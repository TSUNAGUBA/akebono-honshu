...
| 認証・権限 | `user`（利用者マスタ） | 担当者選択・**5**権限カテゴリ（2026-07-27 更新: 4 + 勤怠 `attendance_permission`）| 論理削除 |
...
| 6 | 価格権限ゲート（`price:read`）の本格導入 | 既存仕入単価と素材単価を一括で横断改修／段階導入。既存 **5** 権限モデル（2026-07-27 更新: 4 + 勤怠）との整合 | 32 / 11 |
