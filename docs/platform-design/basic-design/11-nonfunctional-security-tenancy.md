...
|--------|--------|-------------|--------|-------------|-----------|
| 仕入単価 / 機微仕入原価 | 中-高 | RDS/Aurora Storage Encryption（KMS）。**列単位暗号化（pgcrypto）は Phase 5 で再評価** | TLS 1.2+ | 4 権限ポリシー + テナント境界 | **既定マスク**、開示は明示フラグ + 権限 + 監査 |
...
- **機微仕入原価はコンプライアンス上の最重要データ。** テナント内でも 4 権限で開示制御し、テナント跨ぎでは集計・キャッシュ・ベクター・スナップショットのいずれでも混在させない（§2.5・§3.3）。
