        ...
        <table class="w-full min-w-[820px]">
          ...
          <tbody>
            ...
            <tr v-for="r in rows" :key="r._key" class="border-b border-gray-100 last:border-0">
              <td class="px-3 py-2"><select v-model.number="r.materialRole" :disabled="!canEdit" class="rounded-md border border-gray-300 px-2 py-1 text-sm"><option v-for="ro in ROLES" :key="ro" :value="ro">{{ materialRoleLabel(ro) }}</option></select></td>
              <td class="px-3 py-2"><select v-model.number="r.materialId" :disabled="!canEdit" class="w-44 rounded-md border border-gray-300 px-2 py-1 text-sm"><option :value="0" disabled>選択</option><option v-for="m in materials" :key="m.id" :value="m.id">{{ m.name }}</option></select></td>
              ...
              <td class="px-3 py-2"><select v-model="r.unit" :disabled="!canEdit" class="rounded-md border border-gray-300 px-2 py-1 text-sm"><option v-for="u in UNITS" :key="u">{{ u }}</option></select></td>
              <td class="px-3 py-2"><select v-model.number="r.recommendedSupplierId" :disabled="!canEdit" class="w-40 rounded-md border border-gray-300 px-2 py-1 text-sm"><option :value="null">(未指定)</option><option v-for="s in suppliers" :key="s.id" :value="s.id">{{ s.code }} {{ s.name }}</option></select></td>
