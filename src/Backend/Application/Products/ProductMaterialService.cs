    ...
    public async Task ReplaceAsync(Guid familyId, ReplaceBomRequest req, Guid actorUserId, CancellationToken ct = default)
    {
        ...
        foreach (var m in req.Materials)
        {
            if (m.RequiredQtyPerUnit <= 0 || string.IsNullOrWhiteSpace(m.Unit))
                throw DomainException.Validation("所要量は正の数、単位は必須です");
            if (m.MaterialRole is < 0 or > 4)
                throw DomainException.Validation("部位区分が不正です");
            if (m.LossRate is < 0 or >= 1)
                throw DomainException.Validation("ロス率は 0 以上 1 未満です");
        }
        var dupKey = req.Materials
            .GroupBy(m => (m.MaterialRole, m.MaterialId))
            .FirstOrDefault(g => g.Count() > 1);
        if (dupKey is not null)
            throw DomainException.UniqueViolation("同一部位に同一素材が重複しています");
    ...
    public async Task<MaterialRequirements> GetRequirementsAsync(Guid familyId, int quantity, CancellationToken ct = default)
    {
        ...
        if (bom.Count == 0)
            throw new DomainException(AkbErrorCodes.MakerBomRequirementMissing, 422,
                "素材構成 (BOM) が未登録です。先に BOM を登録してください", "対象品番の BOM (素材構成) を先に登録してください");
