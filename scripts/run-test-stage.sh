...
#
# GitHub Actions（.github/workflows/deploy.yml）のテストゲート各ステージ
# （環境セットアップ・単体・結合・シナリオ）から呼び出される共通実行ラッパー。
# product-plan-agent の同名スクリプトを移植（スタックに依存しない汎用実装）。
#
#   使い方:
#     STAGE_CMD="dotnet build ..." bash scripts/run-test-stage.sh "単体テスト" unit-test
#
#   引数:
#     $1 = ステージ表示名（例: 単体テスト）
...
# 空白のみのコマンドも「未設定」として扱う（空白値の登録でステージが実処理なしに
# 成功扱いとなる空振りを防ぐ・fail-closed）
if [ -z "$(printf '%s' "${STAGE_CMD:-}" | tr -d '[:space:]')" ]; then
