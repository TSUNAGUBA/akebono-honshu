...
--     注: ここで更新するのは画面表示用の品名・明細スナップショット文言のみ。発注ヘッダの
--     宛名スナップショット (supplier/customer name) は発注時点の凍結値として保持する
--     (スナップショット不変性の原則、04-orders.sql §5.2)。
-- ────────────────────────────────────────────────────────────────────────────
