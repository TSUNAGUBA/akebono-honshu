...
| data-design.md | 2026-07-27 | データ設計 (18マスタ + 商品4 + 発注3 + 監査1 + **勤怠6** = **32テーブル**, 正規化, 機密度, ボリューム見積) | 946 |
| api-design.md | 2026-07-27 | API 設計 (21機能 × REST エンドポイント + **勤怠 27 エンドポイント (§2.7)**, 共通規約, 10主要フロー検証) | 1721 |
| screen-design.md | 2026-07-27 | 画面設計 (サイトマップ + **29画面** + 共通レイアウト + レスポンシブ + アクセシビリティ) | 1246 |
...
- **現在ステータス:** COMPLETED (全レビュー観点 36件すべてクローズ、Phase 5 確定)
  - **Iteration 30 (勤怠移植、2026-07-27) で登載した残課題**（`screen-design.md §3.16`）:
    - **決着済み（2026-07-27 オペレーター判断）:** **C-1**（日跨ぎ夜勤の退勤打刻不可）= 夜勤運用が無いため
      **制約として受け入れ** / **OD-1**（BOM 空上書きによるデータ消失）= **記録のみ**（起票せず、
      BOM 改修時の申し送りとして残す）
    - **判断待ち:** **C-2**（打刻修正の対象が同種の先頭 1 件に固定）/ **C-3**（締め日・フレックスが
      集計に未反映。**設定画面の説明文が実態と矛盾**）/ **C-4**（週 40 時間超の法定外残業が未計上）/
      **OD-2**（発注書の連絡文章がテンプレセレクタの「（選択しない）」で**無言上書きされる**。
      `OD-1` と同じデータ消失クラス。修正する場合は 1 行）
