...
| 要素 | 内容 |
|---|---|
| 入力 | 各 SKU に対して仕入先・単価・通貨・有効開始日・決定日のセット（複数可、行追加ボタン）|
| UI | DataTable 形式、行追加 / 削除アクション。仕入先 Combobox は `suppliers` から |
| API | 「保存」で各行 `POST /products/{productId}/supplier-prices` を順次呼出（バッチエンドポイント新設は Phase 5 後半で検討）|
...
|---|---|
| 表示 | Step 1〜3 の入力サマリ |
| アクション | 「登録」→ 全データ確定 → `/orders/new?from_family={id}` へ遷移（UC-1 直結） |
| キャンセル | 確認ダイアログ → 戻る |
