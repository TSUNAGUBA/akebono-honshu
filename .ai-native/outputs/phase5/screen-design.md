...
| モバイル（原則8）| 一覧は PC = テーブル（`hidden md:block`）/ モバイル = カード（`md:hidden`）の二重描画。タブバーは横スクロール、2 カラム領域は `lg:grid-cols-2`、KPI は `grid-cols-2 sm:grid-cols-3`。**モバイルカード内の操作リンク（特に削除等の破壊的操作）は `inline-flex min-h-[44px] items-center` で 44px 相当のタップ領域を確保**（下記）|
...
**取得失敗時は集計を描画しない（2026-07-27 追記、コミット `e7d0fbd`）:**
...
**モバイルの破壊的操作のタップ領域（2026-07-27 追記、コミット `e7d0fbd`、原則8）:**
...
> **PC 側の高密度レイアウトは変更しない。** テーブル表示は視認とマウス操作が前提で 1 行が高くなると一覧性が落ちるため、拡大はモバイルカード側の導線に限定する。
...
| 空状態 | 「この期間の打刻がありません」。**初回取得が完了するまでは表示しない**（`listLoading` の初期値を `true` にして「読み込み中…」を先に出す。`false` 始まりにすると初回取得が終わるまでの一瞬だけ**偽の空状態**が出る。`reloadRange` の `finally` で必ず `false` に戻る）|
...
| `/attendance` | タブバーは横スクロール。週次・休暇履歴・全員のタイムカード・休暇管理・設定の各一覧はテーブル（`hidden md:block`）とカード（`md:hidden`）の二重描画で、モバイルはカード型。2 カラム領域は `lg:` で分割、KPI/6 区分は `grid-cols-2 sm:grid-cols-3`。**モバイルカード内の操作リンク（削除等の破壊的操作を含む）は `min-h-[44px]` で 44px 相当のタップ領域を確保**（PC 側テーブルの高密度レイアウトは維持。§3.14 参照）|
