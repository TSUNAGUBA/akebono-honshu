...
| 2026-07-30 | **Iteration 33: 勤怠承認経路（akebono-office からの移植）。** §14 に 4 テーブルを追加: `attendance_routes`（区分 direct/fix × 承認経路）/ `attendance_route_steps`（経路ステップ定義。承認者を 役職/ロール/個人 で指定）/ `direct_requests`（直行/直帰申請）/ `attendance_request_steps`（申請時に凍結する経路スナップショット。direct/fix 共通の soft reference）。`attendance_fix_requests` に `current_step`（多段承認の現在ステップ）/ `direct_request_id`（直行/直帰起因の紐付け）を追加し、`status` の CHECK を 0..2 → 0..3（`in_review` 追加）へ拡張。§3.18 `users` に `title`（役職。承認経路の approver_type=title が参照）を追加。すべて追加列・制約緩和のみで下位互換（経路未設定の区分はオーナー 1 名の単段承認へフォールバック＝従来挙動）。`db/init/11-attendance-approval-routing.sql` と `db/migration/iter33-attendance-approval-routing.sql` を同時整備し、**両ファイルを正規化 diff で DDL 完全一致（差分検証）**。起動時スキーマガードの検査対象に `users.title` / `attendance_fix_requests.{current_step, direct_request_id}` を追加。承認者解決（`ApproverResolver.Pick`）と経路解決（`AttendanceRouteResolver.Resolve`）は Domain 純関数として単体テスト（office の approver.test.ts / attendance-route.test.ts 移植）|
