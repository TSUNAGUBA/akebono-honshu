...
> **Probe de-dup の信頼性:** `Auth.LoginRejected.Inactive` / `Auth.UidUnboundProbe` の 5 分 atomic de-dup は同一プロセス内では有効だが、(a) multi-instance ではインスタンス毎に独立して de-dup されるため記録回数 = N (instance 数) 、(b) memory pressure による cache eviction で de-dup キー (`audit_logged:{uid}`) が蒸発すると再記録される。段階 C で multi-instance + 高負荷化する際は ElastiCache 共有または DB ベースのレートリミットへの置換を検討。
...
> **soft-deleted (`IsDeleted=true`) ユーザの扱い (設計判断、4 周目レビュー反映):** OnTokenValidated の引当 WHERE は `!IsDeleted` のため、soft-deleted ユーザは引当でも hit せず `(ActiveId=null, AnyId=null)` の組合せとなり `Auth.UidUnboundProbe` (actor_user_id=null) として記録される。これは「Firebase Auth `disabled=true` 同期を SoT 防御の単一ポイント」とする設計判断による:
...
> 個別追跡を厳密化するなら引当 WHERE に `IgnoreQueryFilters()` + 新 action `Auth.LoginRejected.Deleted` を追加するが、Firebase 側防御で十分という現運用前提で本設計を採択。
