...
| # | 欠落要素 | 根拠 |
|---|---------|------|
| 1 | **工場マスタ** | Excel「ホンシュ_品番設定 コード一覧.xlsx」に 27 件超の工場リストが存在。11桁品番の 7 桁目を構成。 |
| 2 | **甲皮素材マスタ** | 既存「品番台帳入力」画面に「甲皮素材」項目あり。17マスタには素材（中底/底素材想定）のみ。 |
| 3 | **色のサブ分類**（値札 1-5、色相 -/+） | Excel 色コード表に存在するが、17マスタの `color` には `name` と `item_conversion_code` しか持たない。これらをどう表現するか要設計（追加カラム or 親子テーブル化）。 |
| 4 | **商品マスタ本体テーブル** | 17マスタは「マスタ」のみで、品番（SKU）レコードを格納する **取引上の本体テーブル** が含まれない。11桁品番1件ごとのレコードを保持する `product` テーブルが必要。 |
| 5 | **年式コードマスタ**（または変換ロジック） | 11桁品番の1桁目（年式）は文字（A-K, N, Z）で表現されるが、17マスタには対応マスタなし。コードロジックとして実装するか、専用マスタを設けるかの判断が必要。 |
| 6 | **Departures（自社工場）の扱い** | ユーザー発言に「Departures（自社工場）」あり。工場マスタの 1 エントリか、別概念（事業部マスタとの組み合わせ）か要確認。 |
...
| # | 項目 | 確認方法 |
|---|------|----------|
| 1 | 仕入先マスタ `item_conversion_code` と工場マスタの関係（同一物か別物か） | 現場ヒアリング |
